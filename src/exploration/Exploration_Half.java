...
public class Exploration_Half {
    private FastestPath fp = new FastestPath();
    private Map map;
    private boolean image_stop;

    public void Exploration_Half(Robot robot, int time, int percentage, int speed, boolean image_recognition){
        map = robot.getMap();
...
//            corner_calibration(robot);
        } while (!at_pos(robot, Constant.END));


        System.out.println("Half Exploration Complete!");
    }
    ...
    private int[][] image_recognition(Robot robot, int[][] checked_obstacles) {
        ...
        for (int k=0; k<3; k++) {
            ...
            if (!within_map(obs_pos[k][0], obs_pos[k][1])) {
                ...
                // Check if the obstacle has been checked
