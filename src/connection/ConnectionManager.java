...
import exploration.ExplorationHalfThread;
...
//				complete = true;
                ...
                s = "Exploration Started";
                thread = ExplorationHalfThread.getInstance(robot, Constant.TIME, Constant.PERCENTAGE, Constant.SPEED, Constant.IMAGE_REC);
                thread.setPriority(Thread.MAX_PRIORITY);
			...
			// Set waypoint only if the correct message is received and the program is not running exploration and fastest path
				...
				Pattern wp_d_s = Pattern.compile(Constant.SETWAYPOINT + " \\([1-9],[1][0-4]\\)");
				Pattern wp_s_d = Pattern.compile(Constant.SETWAYPOINT + " \\([1][0-8],[1-9]\\)");
				Pattern wp_d_d = Pattern.compile(Constant.SETWAYPOINT + " \\([1][0-8],[1][0-4a]\\)");
